...
public interface ServiceDao {
    ...
    @Query("select MIN(time) from service where customer_username = :username")
    Date getEarliestFinishedServiceCustomer(String username);

    @Query("select * from service where customer_username = :username and time >= :firstOfMonth and time <= :lastOfMonth group by time order by time desc")
